...
class NewsComments extends Component{
    ...
    submitComment=()=>{
        ...
        //发送ajax请求
        let {uniquekey}=this.props
        let {content}=this.props.form.getFieldsValue()
        let url=`http://newsapi.gugujiankong.com/Handler.ashx?action=comment&userid=${userId}&uniquekey=${uniquekey}&commnet=${content}`
        axios.get(url)
            .then(response=>{
